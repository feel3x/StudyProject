...
function VIEWSCOREBOARDrefresh<?php echo $panelId; ?>(element)
{
   ...
   timeout = setTimeout(function() {  refreshPanel("<?php echo $panelId; ?>"); }, 10000);
...
<!--     <div class="col-sm"><input type="checkbox"  name="scrollWithComp<?php echo $panelId; ?>" id="scrollWithComp<?php echo $panelId; ?>" class="scrollWithCompetitor" CHECKED>
...
/*$stmt = $db->prepare("SELECT
competitorId, 
AVG(CASE WHEN criteria = 'form' THEN value ELSE 0 end) AS sumform, 
SUM(CASE WHEN criteria = 'difficulty' THEN value ELSE 0 end) AS sumdiff, 
SUM(CASE WHEN criteria = 'tof' THEN value ELSE 0 end) AS tof, 
$scoreRules AS score  
FROM scores LEFT JOIN user ON scores.competitorId LIKE user.id WHERE disciplineId LIKE ? GROUP BY competitorId ORDER BY score DESC");
 */     $stmt->execute(array_merge(array($disId), $associatedMembersOfDis));
      ...
      while($scoreRow = $stmt->fetch(PDO::FETCH_ASSOC))
      {
          $startNumber++;
?>
       <tr class="<?php if($execDisInfo != false && $execDisInfo['currentAthlete'] == $scoreRow['id']) { echo 'table-success text-dark'; } ?>">
        ...
        <td>
        <?php  if($execDisInfo != false && $execDisInfo['currentAthlete'] == $scoreRow['id'])
{
  //create scroll anchor
  ?>
 <!-- <a id="<?php echo $panelId; ?>Scrollanchor"></a>
  <script>  
  setTimeout(function() { if(document.querySelector('#scrollWithComp<?php echo $panelId; ?>').checked)
  {
  document.querySelector('#<?php echo $panelId; ?>Scrollanchor').scrollIntoView({
            block: 'center',
            inline: 'center'
        });
  }}, 1);
...
?>    
        <?php echo $scoreRow['firstName']." ".$scoreRow['lastName']; ?></td>
